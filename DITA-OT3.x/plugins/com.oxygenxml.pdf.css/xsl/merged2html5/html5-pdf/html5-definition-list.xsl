...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- The DLENTRY ID is copied to the DT (the definition term),
         so is no need to duplicate it on the DD. 
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="result">
                    <root>
                      <!-- The @id may come from commonattributes applied to the parent DLENTRY. This has to be removed, is already set on the DT. -->                
                      <xsl:next-match/>
                    </root>
                </xsl:variable>
